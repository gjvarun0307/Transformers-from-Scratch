...
class BilingualDataset(Dataset):
    ...
    def __init__(self, ds, tokenizer_src, tokenizer_trg, src_lang, tgt_lang, seq_len):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        dec_input_tokens = self.tokenizer_trg.encode(tgt_text).ids
        ...
        return {
            ...
            "decoder_mask": (decoder_input != self.pad_token).unsqueeze(0).unsqueeze(0).int() & casual_mask(decoder_input.shape[0]),     # The casual_mask is the special mask for decoder
            "label": label,
            "src_text": src_text,
            "tgt_text": tgt_text
        }
